...
class DriftAnalysisEngine:
    ...
    def _load_anchor(self) -> Optional[List[float]]:
        """Load the persisted baseline vector if available."""
        try:
    ...
    def _load_last_report(self) -> Optional[List[float]]:
        """Load the last report vector if available."""
        try:
    ...
    def _save_last_report(self, vector: List[float]) -> None:
        """Persist the given vector as the latest report."""
    ...
    def _log_report(
        ...
        """Write a detailed drift report to the logs directory."""
    ...
    def analyze_input(self, quality_score: float, tags: Set[str]):
        """Analyze an input and check for drift.

        Parameters
        ----------
        quality_score : float
            Normalized quality or truthfulness score between 0.0 and 1.0.
        tags : Set[str]
        ...
        tuple[list[float], bool]
            The computed truth vector and whether a drift alarm was triggered.
        """
        try:
    ...
    def rotate_anchor(self, new_anchor: Optional[List[float]] = None) -> None:
        """Update the baseline truth vector.

        Parameters
        ----------
        new_anchor : list of float, optional
            Four-dimensional vector to use as the new baseline. If ``None``, the
            last processed vector becomes the new baseline.

        Raises
        ------
        ValueError
            If no appropriate vector is available or the provided vector does not
            have four elements.
        """
