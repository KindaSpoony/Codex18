...
def run_ingest(tmp_path: Path, fixture_name: str):
    ...
    report_path = incoming / fixture.name
    report_path.write_text(fixture.read_text())
...
def test_yaml_metadata_parsing(tmp_path):
    ...
    assert any(p.name.startswith("report_with_yaml") for p in archive.iterdir())
...
def test_json_output_file_creation(tmp_path):
    ...
    assert data["content"].strip().startswith("Just some report content")
    assert data["ingest_timestamp"].endswith("Z")
    parser.isoparse(data["ingest_timestamp"])  # will raise if invalid
...
def test_content_hash_and_timestamp(tmp_path):
    ...
    out_file = output / json_name
    data = json.loads(out_file.read_text())
    expected_hash = hashlib.sha256("Just some report content without metadata.".encode("utf-8")).hexdigest()
    assert data["sha256"] == expected_hash
    ts = parser.isoparse(data["ingest_timestamp"])
